...
export class QatarAirwaysApi implements AirlineApi {
  ...
  async searchFlights(params: FlightSearchParams): Promise<any[]> {
    try {
      ...
      logger.error('Error searching flights with Qatar Airways API', error as Error, { params });
      throw error;
    }
  }
  ...
  async getFlightStatus(flightId: string): Promise<FlightStatus> {
    try {
      ...
      logger.error('Error getting flight status from Qatar Airways API', error as Error, { flightId });
      throw error;
    }
  }

  private mapToFlightStatus(data: any): FlightStatus {
    // Implement mapping logic from Qatar Airways API response to FlightStatus
    return {
