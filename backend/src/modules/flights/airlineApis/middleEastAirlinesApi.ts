...
export class MiddleEastAirlinesApi implements AirlineApi {
  ...
  async searchFlights(params: FlightSearchParams): Promise<any[]> {
    try {
      ...
      logger.error('Error searching flights with Middle East Airlines API', error as Error, { params });
      throw error;
    }
  }
  ...
  async getFlightStatus(flightId: string): Promise<FlightStatus> {
    try {
      ...
      logger.error('Error getting flight status from Middle East Airlines API', error as Error, { flightId });
      throw error;
    }
  }

  private mapToFlightStatus(data: any): FlightStatus {
    // Implement mapping logic from Middle East Airlines API response to FlightStatus
    return {
