...
export class AirArabiaApi implements AirlineApi {
  ...
  async searchFlights(params: FlightSearchParams): Promise<any[]> {
    try {
      ...
      logger.error('Error searching flights with Air Arabia API', error as Error, { params });
      throw error;
    }
  }
  ...
  async getFlightStatus(flightId: string): Promise<FlightStatus> {
    try {
      ...
      logger.error('Error getting flight status from Air Arabia API', error as Error, { flightId });
      throw error;
    }
  }

  private mapToFlightStatus(data: any): FlightStatus {
    // Implement mapping logic from Air Arabia API response to FlightStatus
    return {
