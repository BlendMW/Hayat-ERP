...
import { MetaSearchResult } from '../models/MetaSearchResult';
import { HayatError } from '../utils/errorHandling';
import { cacheSet, cacheGet } from './cacheService';
...
export const performMetaSearch = async (searchParams: SearchParams): Promise<MetaSearchResult[]> => {
  ...
  const activeProviders = await ExternalProvider.scan({ filter: { isActive: { eq: true } } });
